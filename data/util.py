...
def augment(img, hflip=True, rot=True, mode=None, swap=None):
    ...
    def _augment(img):
        if hflip:
            img = img[:, ::-1] if img.ndim == 2 else img[:, ::-1, :]
        if vflip:
            img = img[::-1, :] if img.ndim == 2 else img[::-1, :, :]
        if rot90:
            img = img.T if img.ndim == 2 else img.transpose(1, 0, 2)
        return img
    if mode in ['LQ','GT', 'SRker']:
        return _augment(img)
    elif mode in ['LQGTker', 'LQGT', 'CondLQGT', 'Cond2LQGT', 'ContextLQGT', 'StereoCondLQGT']:
