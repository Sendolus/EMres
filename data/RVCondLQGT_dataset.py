...
class RVCondLQGTDataset(data.Dataset):
    ...
    def __init__(self, opt):
        super().__init__()
        self.opt = opt
        self.LR_paths, self.GT_paths, self.mask_paths = None, None, None
        ...
        if opt["data_type"] == "lmdb":
            ...
            self.mask_paths, self.mask_sizes = util.get_image_paths(
                opt["data_type"], opt["dataroot_mask"]
            )
        ...
        if self.LR_paths and self.mask_paths:
            assert len(self.LR_paths) == len(
                self.mask_paths
            ), "mask and LR datasets have different number of images - {}, {}.".format(
                len(self.LR_paths), len(self.mask_paths)
            )
    ...
    def __getitem__(self, index):
        if self.opt["data_type"] == "lmdb":
            if (self.GT_env is None) or (self.LR_env is None) or (self.mask_env is None):
                self._init_lmdb()
        ...
        if self.LR_paths:  # LR exist
            ...
            mask_path_L = self.mask_paths[index*2]
            mask_path_R = self.mask_paths[index*2+1]
            if self.opt["data_type"] == "lmdb":
                resolution = [int(s) for s in self.LR_sizes[index].split("_")]
            else:
                resolution = None
            img_LR_L = util.read_img(self.LR_env, LR_path_L, resolution)
        ...
        if self.opt["phase"] == "train":
            ...
            img_mask_L = img_mask_L[rnd_h: rnd_h + LR_size, rnd_w: rnd_w + LR_size]
            img_mask_R = img_mask_R[rnd_h: rnd_h + LR_size, rnd_w: rnd_w + LR_size]
        ...
        if len(img_mask_R.shape) == 3:
            img_mask_R = img_mask_R[:, ::-1, :]
        else:
            img_mask_R = img_mask_R[:, ::-1]
        ...
        img_mask = torch.cat([img_mask_L, img_mask_R], dim=0)

        if LR_path_L is None:
            LR_path_L = GT_path_L

        return {"LQ": img_LR, "GT": img_GT, "mask": img_mask, "LQ_path": LR_path_L, "GT_path": GT_path_L, "mask_path": mask_path_L}
