...
class StyleLQGTDataset(data.Dataset):
    """
    ...
    def __init__(self, opt):
        super().__init__()
        self.opt = opt
        self.LR_paths, self.GT_paths, self.style_paths = None, None, None
        self.LR_env, self.GT_env, self.style_env = None, None, None  # environment for lmdb
        ...
        if opt["data_type"] == "lmdb":
            ...
            self.style_paths, self.style_sizes = util.get_image_paths(
                opt["data_type"], opt["dataroot_style"]
            )
            ...
            self.style_paths = util.get_image_paths(
                opt["data_type"], opt["dataroot_style"]
            )
        else:
        ...
        if self.LR_paths and self.style_paths:
            assert len(self.LR_paths) == len(
                self.style_paths
            ), "style and LR datasets have different number of images - {}, {}.".format(
                len(self.LR_paths), len(self.style_paths)
            )
    ...
    def _init_lmdb(self):
        ...
        self.style_env = lmdb.open(
            self.opt["dataroot_style"],
            readonly=True,
            lock=False,
            readahead=False,
            meminit=False,
    ...
    def __getitem__(self, index):
        if self.opt["data_type"] == "lmdb":
            if (self.GT_env is None) or (self.LR_env is None) or (self.style_env is None):
                self._init_lmdb()

        GT_path, LR_path, style_path = None, None, None
        scale = self.opt["scale"] if self.opt["scale"] else 1
        GT_size = self.opt["GT_size"]
        LR_size = self.opt["LR_size"]
        style_size = self.opt["LR_size"]
        ...
        if self.LR_paths:  # LR exist
            ...
            img_style = util.read_img(self.style_env, style_path, resolution)
        else:  # down-sampling on-the-fly (NOT IMPLEMENTED style HERE)
        ...
        if self.opt["phase"] == "train":
            ...
            img_style = img_style[rnd_h : rnd_h + LR_size, rnd_w : rnd_w + LR_size]
            ...
            img_LR, img_GT, img_style = util.augment(
                [img_LR, img_GT, img_style],
                self.opt["use_flip"],
                self.opt["use_rot"],
                self.opt["mode"],
                self.opt["use_swap"],
        ...
        img_style = torch.from_numpy(
            np.ascontiguousarray(np.transpose(img_style, (2, 0, 1)))
        ).float()
        if LR_path is None:
            LR_path = GT_path

        return {"LQ": img_LR, "GT": img_GT, "style": img_style, "LQ_path": LR_path, "GT_path": GT_path, "style_path": style_path}
