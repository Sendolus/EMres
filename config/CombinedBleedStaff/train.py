...
def main():
    ...
    if resume_state:
        ...
        for param_group in model.optimizers[0].param_groups:
            param_group['lr'] = 2.5e-5  # For example: 1e-5
    else:
        current_step = 0
        start_epoch = 0
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            ...
            LQ, GT, mask = train_data["LQ"], train_data["GT"], train_data["mask"].to(device)
            timesteps, states = sde.generate_random_states(x0=GT, mu=LQ)

            model.feed_data(torch.cat([states, mask], dim=1), LQ, GT) # xt, mu, x0
            ...
            if current_step % opt["train"]["val_freq"] == 0 and rank <= 0:
                ...
                for _, val_data in enumerate(val_loader):
                    ...
                    LQ, GT, mask = val_data["LQ"], val_data["GT"], val_data["mask"].to(device)
                    noisy_state = sde.noise_state(LQ).to(device)

                    # valid Predictor
                    model.feed_data(torch.cat([noisy_state, mask], dim=1), LQ, GT)
                    model.test(sde)
