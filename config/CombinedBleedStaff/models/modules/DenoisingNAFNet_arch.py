...
class SCAM(nn.Module):
    ...
    def forward(self, x):
        ...
        x_ls = F.interpolate(x_l, scale_factor=0.25, mode='bicubic')
        x_rs = F.interpolate(x_r, scale_factor=0.25, mode='bicubic')

        Q_l = self.l_proj1(self.norm_l(x_ls)).permute(0, 2, 3, 1)  # B, H, W, c
        Q_r_T = self.r_proj1(self.norm_r(x_rs)).permute(0, 2, 1, 3) # B, H, c, W (transposed)

        V_l = self.l_proj2(x_ls).permute(0, 2, 3, 1)  # B, H, W, c
        V_r = self.r_proj2(x_rs).permute(0, 2, 3, 1)  # B, H, W, c
        ...
        F_r2l = F.interpolate(F_r2l, size=x_l.shape[2:])
        F_l2r = F.interpolate(F_l2r, size=x_r.shape[2:])

        xl = F_r2l + x_l
        xr = F_l2r + x_r
        return torch.cat([xl, xr], dim=0).contiguous()
...
class ConditionalNAFNet(nn.Module):
    ...
    def forward(self, inp, cond, time):
        ...
        xtl, xtr = torch.chunk(inp, 2, dim=1)
        condl, condr = torch.chunk(cond, 2, dim=1)

        xtl = xtl - condl
        xtr = xtr - condr
        xl = torch.cat([xtl, condl], dim=1)
        xr = torch.cat([xtr, condr], dim=1)
