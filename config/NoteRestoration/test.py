...
Modifications: Incorporation of additional context examples
"""
import argparse
import logging
import os.path
...
if os.path.exists(result_path):
    os.remove(result_path)  # Remove file

# Create a symbolic link (requires admin privileges on Windows)
target = os.path.abspath(os.path.join(opt["path"]["results_root"], ".."))
try:
    os.symlink(target, result_path)
except OSError:
    print("Symbolic links require administrator privileges on Windows. Trying a directory copy instead.")
    shutil.copytree(target, result_path)
...
for test_loader in test_loaders:
    ...
    for i, test_data in enumerate(test_loader):
        ...
        #### input dataset_LQ
        LQ, GT, context1, context2, context3 = test_data["LQ"], test_data["GT"], test_data[
                        "context1"].to(
                        device), test_data["context2"].to(device), test_data["context3"].to(device)
        noisy_state = sde.noise_state(LQ)

        model.feed_data(torch.cat([noisy_state.to(device), context1, context2, context3], dim=1), LQ, GT)
        tic = time.time()
        ...
        if need_GT:
            ...
            lp_score = lpips_fn(
                GT.to(device) * 2 - 1, SR_img[:3, :, :].to(device) * 2 - 1).squeeze().item()
