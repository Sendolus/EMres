...
def main():
    ...
    parser.add_argument("-opt", type=str, default=r"options/train/ir-sde.yml", help="Path to option YMAL file.")
    parser.add_argument(
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            ...
            LQ, GT, context1, context2, context3 = train_data["LQ"], train_data["GT"], train_data["context1"], \
                                                   train_data["context2"], train_data["context3"]

            timesteps, states = sde.generate_random_states(x0=GT, mu=LQ)

            model.feed_data(torch.cat([states, context1.to(device), context2.to(device), context3.to(device)], dim=1),
                            LQ, GT)
            ...
            if current_step % opt["train"]["val_freq"] == 0 and rank <= 0:
                ...
                for _, val_data in enumerate(val_loader):
                    LQ, GT, context1, context2, context3 = val_data["LQ"], val_data["GT"], val_data[
                        "context1"].to(
                        device), val_data["context2"].to(device), val_data["context3"].to(device)
